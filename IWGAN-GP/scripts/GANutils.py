...
def load_values(save_dir, recon = False, valid = False):
    ...
    return outputs 
def render_graphs(save_dir,epoch, track_d_loss_iter, track_d_loss,track_g_loss_iter,track_g_loss,track_recon_loss_iter = None, track_recon_loss=None, track_valid_loss_iter=None, track_valid_loss=None):
    if not os.path.exists(save_dir+'/plots/'):
        os.makedirs(save_dir+'/plots/')
    if not os.path.exists(save_dir+'/excel/'):
        os.makedirs(save_dir+'/excel/')
    ...
    if len(track_d_loss)> 51: 
        ...
        saveexcel(track_d_loss_iter,save_dir+'/excel/d_x.xlsx')
        saveexcel(track_d_loss, save_dir + '/excel/d_loss_y.xlsx')
        saveexcel(smoothed_d_loss, save_dir + '/excel/smoothde_d_y.xlsx')
        saveexcel(track_g_loss_iter, save_dir + '/excel/g_x.xlsx')
        saveexcel(track_g_loss, save_dir + '/excel/g_loss_y.xlsx')
        saveexcel(smoothed_g_loss, save_dir + '/excel/smoothde_g_y.xlsx')
